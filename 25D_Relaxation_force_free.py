...
# parameters 
output = True # whether we print or save?
Lx, Ly = 8, 8
Nx, Ny = 16, 16
...
Z = MixedFunctionSpace([*Vd, *Vc, *Vc, *Vc, *Vc])
z = Function(Z)
(Bper, Bpar, jper, jpar, Hper, Hpar, uper, upar, Eper, Epar) = split(z)
B = (Bper, Bpar)
j = (jper, jpar)
H = (Hper, Hpar)
u = (uper, upar)
...
# Test functions
(Bper_t, Bpar_t, jper_t, jpar_t, Hper_t, Hpar_t, uper_t, upar_t, Eper_t, Epar_t) = split(TestFunction(Z))
B_t = (Bper_t, Bpar_t)
j_t = (jper_t, jpar_t)
H_t = (Hper_t, Hpar_t)
u_t = (uper_t, upar_t)
...
def curl_(X_):
    """
    Function that takes in array of scalar + vec corresponding to perp component + parallel component.
    Output the same...
    """
    return (curl(X_[1]), rot(X_[0]))
def cross_(X_, Y_):
    """
    Cross product of objects that are numpy arrays of scalar + vec 
    """
    return (
...
pvd = VTKFile("output/boris_output.pvd")
